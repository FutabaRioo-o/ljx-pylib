...
// Static variable
static int sockfd[ MAX_LJXA_DEVICENUM ] = { -1, -1, -1, -1, -1, -1 };
static int sockfdHighSpeed[ MAX_LJXA_DEVICENUM ] = { -1, -1, -1, -1, -1, -1 };
...
static unsigned short GetCurrentProfileUnitForFallback(int lDeviceId);
...
int LJX8IF_PreStartHighSpeedDataCommunication(int lDeviceId, LJX8IF_HIGH_SPEED_PRE_START_REQ* pReq, LJX8IF_PROFILE_INFO* pProfileInfo)
{
	...
	// Some controller/firmware combinations return zero for the unit in the
	// pre-start response. Preserve the unit from a readable current profile so
	// simple-array conversion can still be initialized correctly.
	unsigned short fallbackProfileUnit = GetCurrentProfileUnitForFallback(lDeviceId);
	...
	if (profileUnit == 0) {
		profileUnit = fallbackProfileUnit;
	}
...
static unsigned short GetCurrentProfileUnitForFallback(int lDeviceId)
{
	// Request one current profile without erasing it. Only the profile unit in
	// the response header is needed; profile conversion is intentionally skipped.
	...
	return *(unsigned short*)(&myrecvline[lDeviceId][54]);
}
